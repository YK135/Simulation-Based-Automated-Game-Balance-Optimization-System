...
def Make_MidBoss(player_lv: int) -> Unit:
    ...
    unit = Unit(
        ...
        grade = "상",
        is_boss      = True,
        # ★ 0.30이었지만 이 필드는 6차 전까지 어디서도 읽히지 않았다(죽은 값).
        #   실제로 켜 보면 Lv8 벽이 더 세진다(N=200 실측: 전사 40.7→37.5,
        #   마법사 39.3→32.0, 도적 86.0→76.0). "Lv8 벽은 그대로 둔다"는
        #   15판 A-1 결정과 어긋나므로 0으로 명시한다 — 되살리려면 그 결정부터.
        debuff_resist = 0.0,
    )
...
def Make_FinalBoss(player_lv: int) -> Unit:
    """
    최종 보스: HP/저항 위주 — 6차 조정(2026-09-21).

    ★ 6차에서 바꾼 것: 5차 스펙(hp 3450 / stg 64 / arm 51 / sparm 58 / sp 62)의
      hp·stg·arm·sparm·sp를 일괄 ×0.72로 낮췄다. spd·luc·mp는 그대로다.

    이유 — 보스가 "도달할 수 없는 레벨"에 맞춰져 있었다. 실제 런의 경험치를
    그대로 굴려 보면(전투 노드 22.6개 + 엘리트 1.3 + 중간 보스, N=120런)
    최종 보스 도착 레벨은 중앙값 Lv20 · 평균 20.6이고 Lv25 도달은 5.8%뿐인데,
    5차 스펙의 Lv20 승률은 전사 2.0% / 마법사 2.5% / 도적 24%였다.
    즉 사실상 모든 런이 최종 보스에서 끝났다. 권장 Lv25(전사 54 / 마법사 58 /
    도적 94%)는 거의 아무도 보지 못하는 숫자였다.

    ×0.72의 근거(N=150/셀, balanced, 시드 20260917):
      Lv18  전사 25.3 / 마법사 28.7 / 도적 50.7
      Lv19  전사 47.3 / 마법사 50.0 / 도적 70.0
      Lv20  전사 50.0 / 마법사 64.7 / 도적 90.7   ← 도착 중앙값
      Lv22  전사 64.7 / 마법사 87.3 / 도적 96.7
      Lv25  전사 98.0 / 마법사 99.3 / 도적 99.3   ← 5.8%만 도달, 승리 보너스로 둔다
    레벨 스케일링은 넣지 않는다 — 보스는 직접 튜닝한다는 기존 원칙(_apply_grade
    우회)을 지키고, 평탄한 수치라야 "더 싸울수록 보스가 쉬워진다"는 단조 관계가
    보장된다. 레벨별 목표 승률을 맞추려 스케일을 끼우면 실측상 Lv21→22에서
    승률이 역전돼(플레이어 성장 곡선이 매끄럽지 않다) 레벨업이 손해가 된다.

    도적이 밴드 위(Lv20 90.7%)인 것은 보스 수치로 고칠 수 없음을 측정으로 확인했다:
    HP ×1.30에도 도적은 −3%p인데 전사는 −49%p, 「종언」을 3→2행동으로 조여도
    도적 −3 / 전사 −34였다. 원인은 스탯 성장 자체다 — Lv25 기준 SPD가
    도적 50.1 vs 전사 23.4 / 마법사 19.1(보스 34)이라 행동 경제가 2.7배 차이나고,
    LUC 52.9로 회피 21% · 크리 26%를 깔고 간다. 이걸 바꾸려면 game/Lv.py의
    직업 성장 곡선을 손대야 하고 그건 일반 전투 전체와 튜너까지 재측정 대상이다.
    의도: 도전적이지만 가능한 보스.
      ...
      - 5차 실측(2개 시드 × 각 60판): 전사 60~67% / 마법사 72~82% /
        도적 80~85% / 탱커 97~100%.
      - 탱커는 생성 화면에서 비활성(2026-09-20) — 측정 대상에서 빠졌다.
    """
    lv = max(25, player_lv)
    unit = Unit(
        name  = "최종 보스",
        ...
        sparm = 42,       # 58   × 0.72
        sp    = 45,       # 62   × 0.72
        spd   = 34,       # 그대로 — 행동 경제를 건드리면 직업 간 격차가 더 벌어진다
        luc   = 40,       # 그대로
        grade = "상",
        is_boss      = True,
        # debuff_resist는 이번(6차)에 처음으로 실제 동작하게 됐다 — 그 전까지는
        # 어디서도 읽지 않는 죽은 필드였다(ai/battle/Entity._resists_debuff 참고).
        # 선언값 0.50을 그대로 켜면 상태이상에 기대는 마법사만 −28.5%p로 무너져
        # (58.0 → 29.5, ×1.00 기준) 직업 격차가 오히려 커졌다. 0.25는 그 절반이고,
        # 전사(상태이상 거의 안 씀)와 도적(출혈 제거해도 82%)에는 거의 영향이 없다.
        debuff_resist = 0.25,
    )
    return unit
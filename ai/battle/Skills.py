...
SKILL_META = {
    ...
    "추진력": {
        # ── 지속 2턴 → 8턴 (BALANCE_PATCH_10, TestFile/sprint_buff_measure.py) ──
        # 버프는 시전 행동에서 1턴이 줄므로 2턴짜리는 **덮는 행동이 1회뿐**이다
        # (걸고 → 한 번 쓰고 → 만료). 게다가 balanced AI는 "내 실효 SPD ≥ 상대 실효 SPD"일 때
        # SPD 버프를 걸기 때문에, 전 직업 중 가장 빠른 도적이 **거의 항상** 이 분기에 들어간다.
        # 그 둘이 겹쳐 도적은 전투의 33~63%를 피해 0인 재시전에 쓰고 있었다
        # (실측 Lv20: 골렘상 11.1행동 중 4.87회, 최종 보스 19.1행동 중 5.46회).
        # 8턴으로 늘리면 시전이 전투당 1회로 떨어지고(낭비 12~30%), 도적 Lv20 최종 보스가
        # 76.7% → 94.7%가 된다 — 스킬을 아예 뺀 상한(94.0%)과 사실상 같다.
        # 효과·비용은 그대로 둔다(6차 흡혈 버프와 같은 레버 — 비용을 깎으면 더 자주 걸어 악화된다).
        # ★ 몬스터판(MONSTER_SKILL_META)은 2턴 그대로 — 엘리트 암살자는 HP 30% 이하에서
        #   전투당 1회만 걸므로 트레드밀이 없다.
        "mp": 13, "type": "buff",
        "buff_stat": "spd", "buff_amount": 0.10, "buff_turns": 8
    },
    # ─────────────────────────────────────────────
    ...
    "원소 폭발": {
        ...
        # ── 계수 1.2 → 2.3 · MP 15 → 17 (후속 ②, TestFile/elem_burst_measure.py) ──
        #   1.2배/MP 15는 **어떤 상황에서도 최선이 될 수 없는** 수치였다. 주입 가능한 원소는 셋뿐인데
        #   (ice→fire · fire→lightning · lightning→fire) 마법사는 Lv4에 세 원소를 전부 갖고,
        #   세 경우 모두 직접 스킬이 계수는 높고 MP는 싸다(파이어볼1 1.5배/MP 10 · 라이트닝1 1.55배/MP 12).
        #   반응 배율은 두 스킬에 똑같이 곱해져 순위를 못 바꾸므로, 실측에서 선택 0.00회 — 완전 사문화였다.
        #   (강제로 쓰게 하면 승률 47.0% → 29.0%, 반응 횟수도 10.8 → 5.6으로 **줄었다** —
        #    한 행동으로 반응 하나를 내는 동안 평소 로테이션은 반응 + 더 큰 피해를 내기 때문.)
        #   MP를 17로 **올린** 이유가 핵심이다: 계수/MP가 파이어볼1(0.150)을 넘으면 기본 AI(balanced)가
        #   이걸 주력으로 바꿔 **자동 밸런싱 기준선이 통째로 움직인다**(실측: 계수 2.3/MP 15면 balanced
        #   최종 보스 승률 52.7% → 76.7%). 2.3/17 = 0.135로 문턱 아래에 두면 balanced는 그대로 파이어볼1을
        #   쓰고, 반응을 읽는 쪽만 **fire가 붙은 대상**에서 이걸 고른다 — 파이어볼은 같은 원소라
        #   중첩만 되고 라이트닝은 계수가 낮은, 이 스킬만 이득인 한 자리다.
        #   ※ 예외 한 곳: **화염 슬라임처럼 fire에 면역인 대상**에서는 파이어볼1이 효율 −1로 배제돼
        #     balanced의 최선도 라이트닝1(0.129)에서 이 스킬(0.135)로 바뀐다. 반응 배율이 두 스킬에
        #     똑같이 곱해지는 구조라 이 자리를 피하는 수치는 없다. 동일 시드 A/B 480전투 중 12건이
        #     전부 이 조합이었고, 브리프 10장이 말한 설계 의도("원소 슬라임 상대로 특히 강하다")와
        #     일치해 받아들였다 — 마법사 전 구간 스윕으로 밴드를 확인했다(BALANCE_PATCH_7.md).
        #     튜닝은 플레이어별(BalanceHook은 세션마다 그 플레이어로 생성)이라 전사·도적에는 안 샌다.
        #   결과: 사용 0.00 → 1.0~2.4회/전투, 측정용 AI 승률 47.0% → 51.5%, 광역기(화염 폭풍) 선택 빈도
        #   불변(2.32 → 2.32 — 브리프가 경고한 광역 슬롯 역전 없음), 반응 횟수 유지.
        #   ★ 최종 보스 페이즈 1의 "같은 원소 −40%"(BossKit.finalboss_element_resist)는 주입 원소가
        #     항상 상대 원소라 구조적으로 피해 간다 — 브리프 4장이 말한 "보스 부착 페이즈의 핵심기"가 여기서 선다.
        "mp": 17, "mult": 2.3, "type": "magical", "hits": 1, "element": "react",
    ...
    "피의 격노": {
        # 전사 Lv11: MP 대신 현재 HP 15%를 지불(max(1, hp×0.85)) → 8턴간 흡혈 25% (10-4 상한 그대로).
        # ── 지속 3턴 → 8턴 (후속 ①, TestFile/vamp_mark_measure.py) ──
        #   3턴 버프는 시전 행동에서 1턴이 줄어 실제로 덮는 행동이 2회뿐이고, 그래서 2행동마다
        #   다시 걸어야 하는 재시전 트레드밀이 생긴다. 실측에서 묶인 비용은 HP가 아니라 **행동**이었다:
        #   비용을 15%→5%로 깎으면 수지(회수/지불)는 1.1~1.4배로 좋아지는데 승률은 오히려 내려갔다
        #   (전사 Lv20 최종 보스 5.0% → 0.5%, 골렘상 전투가 6.2턴 → 11.3턴). 시전 횟수가 늘어서다.
        #   지속만 8턴으로 늘리면 전투당 시전이 1회로 떨어지고, 수지가 전투 길이에 따라 갈린다 —
        #   긴 전투 1.15~1.58배 / 짧은 전투 0.79~0.86배. Lv25 최종 보스 승률 61.0% → 77.0%.
        #   비율(25%)·지불(15%)·상한(타격 4% · 시전 12%)은 그대로 둔다 — 레버는 지속 하나다.
        "mp": 0, "hp_cost_ratio": 0.15, "type": "buff",
        "buff_stat": "lifesteal", "buff_amount": 0.25, "buff_turns": 8,
    },
    "패 고치기": {
    ...
    "피의 맹세": {
        # Lv25: 8턴간 흡혈 40%, 종료 시 현재 HP 20% 지불(절대 죽지 않음)
        # ── 지속 3턴 → 8턴 (후속 ①, 근거는 「피의 격노」 주석) ──
        #   3턴에서는 수지가 0.39~0.47배로 못 갚았다. 8턴으로 늘리면 Lv25 최종 보스에서
        #   1.41배 · 승률 61.0% → 65.0%.
        #   ★ 지속을 늘리면 "종료 시 지불"이 전투보다 오래 살아 **대가를 아예 안 내고 끝나는**
        #     구멍이 생긴다(실측: 8턴 상태에서 중간 보스전 지불 0 · 암살자상 1). 그래서
        #     BattleSession이 전투가 끝나는 모든 분기에서 Entity.settle_buff_costs()로 정산한다
        #     (ai/Battlesession.py _close_out_battle). 지속을 다시 짧게 돌리더라도 그 정산은
        #     남겨두는 편이 맞다 — 전투가 버프보다 먼저 끝나는 경우는 3턴에서도 생긴다.
        "mp": 12, "type": "buff", "buff_stat": "lifesteal_oath", "buff_amount": 0.40, "buff_turns": 8,
        "expire_hp_cost": 0.20,
    },
    # 마법사
    "연쇄 번개": {
    ...
    # 도적
    "약점 표식": {
        # Lv11: 대상에게 8턴 표식 — 주사위 5도 크리 처리 + 받는 피해 +8%
        # ── 지속 3턴 → 8턴 (후속 ①, 근거는 「피의 격노」 주석과 같은 측정) ──
        #   이 표식은 대상의 행동으로 줄어들어 3턴이면 도적 행동 4~5회를 덮는다. 한 행동을
        #   내고 얻는 이득이 +16%p 남짓이라 그 정도로는 행동값을 못 갚고, 3턴마다 다시 걸게 되어
        #   전투 하나에 4.6회까지 시전했다(Lv20 최종 보스 승률 66.5% → 45.5%).
        #   실측 비교: 받는 피해를 8%→20%로 올려도 −14.5%p(시전 4.4회), 15%+4턴도 −4.5%p인데
        #   **8%를 그대로 두고 8턴으로만 늘리면 +1.5%p**(시전 1.8회)다 — 문제는 세기가 아니라
        #   시전 횟수였다. 받는 피해(8%)와 MP(11)는 그대로 둔다.
        "mp": 11, "type": "debuff", "debuff_stat": "vulnerable",
        "debuff_amount": (0.08, 0.08), "debuff_turns": (8, 8),
    },
    "혈흔 추적": {
